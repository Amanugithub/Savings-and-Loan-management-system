...
import { KeyboardAvoidingView, Platform, Pressable, StyleSheet, Text as RNText, TextInput, View } from 'react-native';
...
export default function Login() { const { colors: themeColors } = useTheme(); s = createStyles(themeColors);
  ...
  async function submit() {
    if (!phone || !password) return setAlert({ title: t('almostThere'), message: t('required') });
    setBusy(true);
    try { await login(phone, password); router.replace('/(tabs)'); }
    catch (e) {
      const message = e instanceof Error ? e.message : '';
      if (message.toLowerCase().includes('inactive')) setAlert({ title: t('membershipInactive'), message: t('inactiveBody') });
      else if (message.toLowerCase().includes('no password')) setAlert({ title: t('passwordNotSet'), message: t('passwordNotSetBody') });
      else setAlert({ title: t('couldNotSignIn'), message: t('incorrect') });
  ...
  return <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={s.page}>
    ...
    <ThemedAlert visible={!!alert} title={alert?.title} message={alert?.message} onClose={() => setAlert(null)} />
