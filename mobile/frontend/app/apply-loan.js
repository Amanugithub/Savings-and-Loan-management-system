...
export default function ApplyLoan() { const { colors: themeColors } = useTheme(); s = createStyles(themeColors);
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => { client.invalidateQueries({ queryKey: ['loans', member?.id] }); Alert.alert('Application sent', 'Your loan application is now pending review.', [{ text: 'View my loans', onPress: () => router.replace('/loans') }]); },
  });
  function submit() {
    if (!principal || principal <= 0) return Alert.alert('Check the amount', 'Enter an amount greater than zero.');
    if (collateral === 'guarantor' && !guarantor) return Alert.alert('Choose a guarantor', 'Enter the member ID of your guarantor.');
    mutation.mutate();
  }
  return <ScrollView style={s.page} contentContainerStyle={s.content}>
    <Pressable onPress={() => router.back()} hitSlop={8} style={s.back}><Ionicons name="arrow-back" size={18} color={colors.green} /><Text style={s.backText}>Back to loans</Text></Pressable>
    ...
    <View style={s.amountInput}><Text style={s.currency}>ETB</Text><TextInput value={amount} onChangeText={setAmount} keyboardType="decimal-pad" placeholder="0.00" placeholderTextColor={colors.subtle} style={s.amountField} /></View>
    ...
    {collateral === 'guarantor' && <><Text style={s.label}>Guarantor member ID</Text><TextInput value={guarantor} onChangeText={setGuarantor} placeholder="Enter member ID" placeholderTextColor={colors.subtle} style={s.input} /></>}
