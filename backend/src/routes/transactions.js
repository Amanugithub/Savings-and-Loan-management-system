  ...
  asyncHandler(async (req, res) => {
    ...
    if (openingBalanceType && loan_id) {
      return res
        .status(400)
        .json({
          error: "Opening balance transactions cannot reference a loan",
        });
    }
    ...
    if (openingBalanceType) {
      ...
      if (existing) {
        return res
          .status(409)
          .json({
            error: "This member already has an opening balance of this type",
          });
      }
    }
  ...
  asyncHandler(async (req, res) => {
    ...
    for (const [name, value] of [
      ...
      if (Array.isArray(value)) {
        return res
          .status(400)
          .json({ error: `${name} must be a single value, not an array` });
      }
    }
    ...
    if (dateFrom && !isValidISODate(dateFrom)) {
      return res
        .status(400)
        .json({ error: "date_from must be a valid date in YYYY-MM-DD format" });
    }
    if (dateTo && !isValidISODate(dateTo)) {
      return res
        .status(400)
        .json({ error: "date_to must be a valid date in YYYY-MM-DD format" });
    }
    if (dateFrom && dateTo && dateFrom > dateTo) {
      return res
        .status(400)
        .json({ error: "date_from must be on or before date_to" });
    }
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (Array.isArray(memberId)) {
      return res
        .status(400)
        .json({ error: "member_id must be a single value, not an array" });
    }
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const fiscalYearResult = parseIntegerParam(fiscalYear, "fiscalYear", {
      min: 1,
    });
    const fiscalMonthResult = parseIntegerParam(fiscalMonth, "fiscalMonth", {
      min: 1,
      max: 12,
    });
