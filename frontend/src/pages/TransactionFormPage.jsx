...
function TransactionFormPage() {
  ...
  return <main className="mx-auto flex w-full max-w-3xl flex-col gap-8"><section><Button variant="ghost" render={<Link to="/transactions" />} className="mb-4 -ml-3"><ArrowLeft data-icon="inline-start" /> Back to transactions</Button><Badge variant="secondary" className="mb-3">Ledger entry</Badge><h1 className="font-heading text-3xl font-semibold tracking-tight text-foreground">Record transaction</h1><p className="mt-2 text-muted-foreground">Choose the transaction type first so the required fields stay relevant.</p></section><Card><CardHeader><CardTitle>Transaction details</CardTitle><CardDescription>All amounts are recorded in Ethiopian birr.</CardDescription></CardHeader><CardContent><form onSubmit={submit} className="flex flex-col gap-6"><div className="grid gap-4 md:grid-cols-2"><label className="flex flex-col gap-2 text-sm font-medium md:col-span-2" htmlFor="type">Transaction type<Select value={form.type} onValueChange={(value) => setForm((current) => ({ ...current, type: value }))}><SelectTrigger id="type" className="w-full"><SelectValue /></SelectTrigger><SelectContent><SelectGroup>{Object.entries(types).map(([value, label]) => <SelectItem key={value} value={value}>{label}</SelectItem>)}</SelectGroup></SelectContent></Select></label>{!organizationLevel && <label className="flex flex-col gap-2 text-sm font-medium md:col-span-2" htmlFor="member_id">Member<Select value={form.member_id || undefined} onValueChange={(value) => setForm((current) => ({ ...current, member_id: value }))}><SelectTrigger id="member_id" className="w-full"><SelectValue placeholder="Select a member" /></SelectTrigger><SelectContent><SelectGroup>{members.map((member) => <SelectItem key={member.id} value={member.id}>{member.name} · {member.phone_number}</SelectItem>)}</SelectGroup></SelectContent></Select></label>}{loanRelated && <label className="flex flex-col gap-2 text-sm font-medium md:col-span-2" htmlFor="loan_id">Loan ID<Input id="loan_id" name="loan_id" value={form.loan_id} onChange={updateField} placeholder="Paste the related loan ID" /></label>}<label className="flex flex-col gap-2 text-sm font-medium" htmlFor="amount">Amount<Input id="amount" name="amount" type="number" min="0.01" step="0.01" value={form.amount} onChange={updateField} required /></label><label className="flex flex-col gap-2 text-sm font-medium" htmlFor="date">Date<DatePicker value={form.date} onChange={(date) => setForm((current) => ({ ...current, date }))} placeholder="Select transaction date" aria-label="Transaction date" /></label><label className="flex flex-col gap-2 text-sm font-medium md:col-span-2" htmlFor="notes">Notes<Input id="notes" name="notes" value={form.notes} onChange={updateField} placeholder="Optional note for the ledger" /></label></div>{mutation.error && <p role="alert" className="rounded-xl bg-destructive/10 px-3 py-2 text-sm text-destructive">{mutation.error.message}</p>}<div className="flex justify-end gap-3"><Button type="button" variant="outline" render={<Link to="/transactions" />}>Cancel</Button><Button type="submit" disabled={mutation.isPending}><Save data-icon="inline-start" />{mutation.isPending ? "Saving…" : "Save transaction"}</Button></div></form></CardContent></Card></main>
}
