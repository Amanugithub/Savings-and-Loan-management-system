...
function TransactionFormPage() {
  ...
  return <main className="mx-auto flex w-full max-w-3xl flex-col gap-8">
    <section><Button variant="ghost" render={<Link to="/transactions" />} className="mb-4 -ml-3"><ArrowLeft data-icon="inline-start" /> Back to transactions</Button><Badge variant="secondary" className="mb-3">Ledger entry</Badge><h1 className="font-heading text-3xl font-semibold tracking-tight text-foreground">Record transaction</h1><p className="mt-2 text-muted-foreground">Loan installment, interest, insurance, and penalty payments are recorded from the loan&apos;s detail page instead — they go through the repayment waterfall there.</p></section>
    <Card><CardHeader><CardTitle>Transaction details</CardTitle><CardDescription>All amounts are recorded in Ethiopian birr.</CardDescription></CardHeader><CardContent><form onSubmit={submit} className="flex flex-col gap-6"><div className="grid gap-4 md:grid-cols-2">
      <label className="flex flex-col gap-2 text-sm font-medium md:col-span-2" htmlFor="type">Transaction type<Select items={availableTypes.map((value) => ({ value, label: typeLabels[value] }))} value={form.type} onValueChange={(value) => setForm((current) => ({ ...current, type: value }))}><SelectTrigger id="type" className="w-full"><SelectValue /></SelectTrigger><SelectContent><SelectGroup>{availableTypes.map((value) => <SelectItem key={value} value={value}>{typeLabels[value]}</SelectItem>)}</SelectGroup></SelectContent></Select></label>
      {!organizationLevel && <label className="flex flex-col gap-2 text-sm font-medium md:col-span-2" htmlFor="member_id">Member<MemberCombobox id="member_id" members={members} value={form.member_id} onValueChange={(value) => setForm((current) => ({ ...current, member_id: value }))} placeholder="Select a member" /></label>}
      ...
      <label className="flex flex-col gap-2 text-sm font-medium" htmlFor="date">Date<DatePicker value={form.date} onChange={(date) => setForm((current) => ({ ...current, date }))} placeholder="Select transaction date" aria-label="Select transaction date" /></label>
      <label className="flex flex-col gap-2 text-sm font-medium md:col-span-2" htmlFor="notes">Notes<Input id="notes" name="notes" value={form.notes} onChange={updateField} placeholder="Optional note for the ledger" /></label>
    </div><WarningsAlert warnings={warnings} />{mutation.error && <p role="alert" className="rounded-xl bg-destructive/10 px-3 py-2 text-sm text-destructive">{mutation.error.message}</p>}<div className="flex justify-end gap-3">{warnings ? <Button type="button" onClick={() => navigate("/transactions")}>Continue</Button> : <><Button type="button" variant="outline" render={<Link to="/transactions" />}>Cancel</Button><Button type="submit" disabled={mutation.isPending}><Save data-icon="inline-start" />{mutation.isPending ? "Saving…" : "Save transaction"}</Button></>}</div></form></CardContent></Card>
  </main>
}
