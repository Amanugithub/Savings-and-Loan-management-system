...
function ExpenseDetailPage() {
  ...
  return <main className="mx-auto flex w-full max-w-3xl flex-col gap-8"><section><Button variant="ghost" render={<Link to="/expenses" />} className="mb-4 -ml-3"><ArrowLeft data-icon="inline-start" /> Back to expenses</Button><Badge variant="outline" className="mb-3">{expense.category}</Badge><h1 className="font-heading text-3xl font-semibold tracking-tight text-foreground">Expense details</h1><p className="mt-2 text-muted-foreground">Recorded on {new Date(`${expense.date}T00:00:00`).toLocaleDateString()}</p></section><Card><CardHeader><CardTitle className="flex items-center gap-2"><CircleDollarSign /> ETB {Number(expense.amount).toLocaleString()}</CardTitle><CardDescription>{expense.description || "No description was provided."}</CardDescription></CardHeader><CardContent className="grid gap-4 sm:grid-cols-2"><Info icon={<CalendarDays />} label="Expense date" value={new Date(`${expense.date}T00:00:00`).toLocaleDateString()} /><Info label="Category" value={expense.category} /><Info label="Expense ID" value={expense.id} /><Info label="Recorded by" value={expense.recorded_by} /></CardContent></Card></main>
}
