...
function ExpensesPage() {
  ...
  return <main className="mx-auto flex w-full max-w-7xl flex-col gap-8">
    ...
    <Card><CardHeader className="gap-4 border-b md:flex-row md:items-end md:justify-between"><div><CardTitle>Expense register</CardTitle><CardDescription className="mt-1">Expenses are ordered from newest to oldest.</CardDescription></div><Select value={filters.category || "all"} onValueChange={(category) => setFilters({ category: category === "all" ? "" : category })}><SelectTrigger className="w-full sm:w-52" aria-label="Filter expenses by category"><SelectValue placeholder="All categories" /></SelectTrigger><SelectContent><SelectGroup>{categories.map((category) => <SelectItem key={category} value={category}>{category === "all" ? "All categories" : labelFor(category)}</SelectItem>)}</SelectGroup></SelectContent></Select></CardHeader><CardContent className="p-0">{isLoading && <p className="p-6 text-sm text-muted-foreground">Loading expenses…</p>}{error && <p className="p-6 text-sm text-destructive">{error.status === 401 ? "Sign in to view expenses." : error.message}</p>}{!isLoading && !error && expenses.length === 0 && <div className="flex flex-col items-center gap-3 p-12 text-center"><CircleDollarSign className="text-muted-foreground" /><p className="text-sm text-muted-foreground">No expenses match the selected category.</p></div>}{!isLoading && !error && expenses.length > 0 && <Table><TableHeader><TableRow><TableHead>Date</TableHead><TableHead>Category</TableHead><TableHead>Description</TableHead><TableHead className="text-right">Amount</TableHead><TableHead className="w-12"><span className="sr-only">Open</span></TableHead></TableRow></TableHeader><TableBody>{expenses.map((expense) => <TableRow key={expense.id}><TableCell className="whitespace-nowrap text-muted-foreground">{new Date(`${expense.date}T00:00:00`).toLocaleDateString()}</TableCell><TableCell><Badge variant="outline">{labelFor(expense.category)}</Badge></TableCell><TableCell className="max-w-80 truncate text-muted-foreground">{expense.description || "—"}</TableCell><TableCell className="text-right font-medium">ETB {Number(expense.amount).toLocaleString()}</TableCell><TableCell><Button variant="ghost" size="icon-sm" render={<Link to={`/expenses/${expense.id}`} />} aria-label="Open expense"><ArrowUpRight /></Button></TableCell></TableRow>)}</TableBody></Table>}</CardContent></Card>
  </main>
}
