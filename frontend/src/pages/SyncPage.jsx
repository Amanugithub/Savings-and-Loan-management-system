...
function SyncPage() {
  ...
  return <main className="mx-auto flex w-full max-w-6xl flex-col gap-8">
    ...
    <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4"><Metric label="Remote status" value={isLoading ? "Checking…" : status?.ok ? "Healthy" : "Degraded"} icon={status?.ok ? CheckCircle2 : TriangleAlert} /><Metric label="Pending local rows" value={pendingRows || "0"} icon={Database} /><Metric label="Last checked" value={status?.checked_at ? formatEthiopianDateTime(status.checked_at) : "—"} icon={Cloud} /><Metric label="Tables tracked" value={tables.length || "—"} icon={Database} /></section>
    ...
    <Card><CardHeader><CardTitle>Remote connection</CardTitle><CardDescription>The service checks the remote PostgreSQL connection before reporting health.</CardDescription></CardHeader><CardContent className="grid gap-4 sm:grid-cols-2"><Info label="Status" value={status?.remote?.ok ? "Connected" : "Unavailable"} /><Info label="Checked at" value={status?.remote?.checked_at ? formatEthiopianDateTime(status.remote.checked_at) : "—"} /><Info label="Remote response" value={status?.remote?.details ? JSON.stringify(status.remote.details) : status?.remote?.error || "—"} /></CardContent></Card>
  </main>
}
