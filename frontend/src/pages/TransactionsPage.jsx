...
function TransactionsPage() {
  ...
  const updateFilter = (event) => setFilters((current) => ({ ...current, [event.target.name]: event.target.value, offset: 0 }))
  ...
  return <main className="mx-auto flex w-full max-w-7xl flex-col gap-8">
    ...
    <Card><CardHeader className="gap-4 border-b"><div><CardTitle>Transaction ledger</CardTitle><CardDescription className="mt-1">Showing {transactions.length} records from the current page.</CardDescription></div><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5"><label className="relative lg:col-span-2"><Search className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground" /><select name="member_id" value={filters.member_id} onChange={updateFilter} className="h-9 w-full appearance-none rounded-4xl border border-input bg-input/30 pl-9 pr-3 text-sm outline-none focus-visible:ring-2 focus-visible:ring-ring" aria-label="Filter by member"><option value="">All members</option>{members.map((member) => <option key={member.id} value={member.id}>{member.name}</option>)}</select></label><select name="type" value={filters.type} onChange={updateFilter} className="h-9 rounded-4xl border border-input bg-input/30 px-3 text-sm outline-none focus-visible:ring-2 focus-visible:ring-ring" aria-label="Filter by transaction type"><option value="">All types</option>{transactionTypes.slice(1).map((type) => <option key={type} value={type}>{typeLabels[type]}</option>)}</select><Input type="date" name="date_from" value={filters.date_from} onChange={updateFilter} aria-label="From date" /><Input type="date" name="date_to" value={filters.date_to} onChange={updateFilter} aria-label="To date" /></div><div className="flex justify-end"><Button type="button" variant="ghost" size="sm" onClick={clearFilters}>Clear filters</Button></div></CardHeader><CardContent className="p-0">
