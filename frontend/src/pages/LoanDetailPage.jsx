...
function LoanDetailPage() {
    ...
    mutation.mutate(
      {
        ...
        status,
        disbursement_date:
          status === "active" ? disbursementDate : undefined,
      },
      {
        onSuccess: () => {
  ...
  const confirmationDescription =
    confirmation === "rejected"
      ...
      : "This will close the loan and mark it as completed. This action cannot be undone."

  const confirmationLabel =
    confirmation === "rejected" ? "Reject loan" : "Close loan"
    ...
    <main className="mx-auto flex w-full max-w-6xl flex-col gap-8">
      <section>
        ...
        <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
          <div>
            <Badge
              variant={statusVariant[loan.status] || "secondary"}
              className="mb-3"
            >
              {loan.status}
            </Badge>
            ...
            <h1 className="font-heading text-3xl font-semibold tracking-tight text-foreground">
              {member?.name || "Loan details"}
            </h1>

            <p className="mt-2 text-muted-foreground">
              {loan.type.replace("_", " ")} · {loan.term_years} year
              {loan.term_years === 1 ? "" : "s"} · {loan.id}
            </p>
          </div>
      ...
      {loan.status === "pending" && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Choose the disbursement date before approving this application.
            </CardDescription>
          </CardHeader>
      ...
      <section className="grid gap-6 md:grid-cols-2">
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Server-calculated values from the application.
            </CardDescription>
          </CardHeader>
          ...
          <CardContent className="grid gap-4 sm:grid-cols-2">
            <Info
              label="Principal"
              value={`ETB ${Number(loan.principal_amount).toLocaleString()}`}
            />
            ...
            <Info
              ...
              value={`ETB ${Number(loan.monthly_installment).toLocaleString()}`}
            />

            <Info
              label="Monthly interest"
              value={`ETB ${Number(loan.monthly_interest_amount).toLocaleString()}`}
            />

            <Info
              label="Insurance"
              value={`ETB ${Number(loan.insurance_amount).toLocaleString()}`}
            />

            <Info
              label="Collateral"
              value={loan.collateral_type}
            />
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Member and collateral information.
            </CardDescription>
          </CardHeader>
          ...
          <CardContent className="grid gap-4">
            <Info
              label="Borrower"
              value={member?.name || loan.member_id}
            />
              ...
              value={
                loan.disbursement_date ? (
                  <span className="font-amharic">
                    {formatEthiopianDate(loan.disbursement_date)}
                  </span>
                ) : (
                  "Not disbursed"
                )
      ...
      <AlertDialog
        ...
        cancelLabel="Cancel"
        confirmLabel={mutation.isPending ? "Processing…" : confirmationLabel}
        onConfirm={() => changeStatus(confirmation)}
        destructive
        disabled={mutation.isPending}
      />
    </main>
